    ...
    if (commentText) {
        ...
        .done(function(comment) {
            ...
            if(!replyTo) {
                ...
                //output the comment from postComment.php->Comment.php->create()
            }
            else {
                $(button).parent().siblings("." + containerClass).append(comment); 
                //ensure text is in correct container class on bottom of page when user posts a reply 
...
function getReplies(commentId, button, videoId) {
    ...
    .done(function(comments) {
        ...
        $(button).replaceWith(replies); //replace 'view all replies' button with replies on click
    });
}